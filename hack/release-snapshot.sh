...
echo "Catalog template: ${CATALOG_TEMPLATE_PATH}"
...
# get the bundle container image from the snapshot
...
IMAGE_VALUE=$(jq -r '(.entries[] | select(.schema == "olm.bundle") | .image)' "${CATALOG_TEMPLATE_PATH}/catalog-template.json")
if [[ -z "${IMAGE_VALUE}" ]]; then
  echo "Error: olm.bundle image cannot be empty in ${CATALOG_TEMPLATE_PATH}/catalog-template.json"
  exit 1
fi
if [[ $(echo "${IMAGE_VALUE}" | wc -w) -ne 1 ]]; then
  echo "Error: olm.bundle image entry must be unique in ${CATALOG_TEMPLATE_PATH}/catalog-template.json"
  exit 1
fi
...
jq --indent 4 --arg image_value "$IMAGE_VALUE" '(.entries[] | select(.schema == "olm.bundle") | .image) = $image_value' \
...
jq -r '(.entries[] | select(.schema == "olm.bundle") | .image)' "${CATALOG_TEMPLATE_PATH}/catalog-template.json"
echo "Updated olm.bundle image hash in ${CATALOG_TEMPLATE_PATH}/catalog-template.json"

# render template and create the catalog.json file
opm alpha render-template \
  basic "${CATALOG_TEMPLATE_PATH}/catalog-template.json" \
  --migrate-level=bundle-object-to-csv-metadata > "${CATALOG_TEMPLATE_PATH}/catalog/windows-machine-config-operator/catalog.json" || {
